...
<div class="pagetitle">
    ...
    <section class="section dashboard">
     ...
     <?php 
      ...
      $objDB = new Database;
      $userid = $_GET['id'];
      $reservation = "SELECT * FROM reservation WHERE userid='$userid' AND (status=0 || status=1) ORDER BY created_at DESC";
            ...
            <div class="row g-0">
              ...
              <div class="col-md-8">
                <div class="card-body">
                  ...
                  <?php if($row_reservation['status'] == 0) { ?>
                    <div class="text-danger mt-2">Pending</div>
                  <?php } ?>
                  <?php if($row_reservation['status'] == 1) { ?>
                    <div class="text-success mt-2">You have successfully accommodated this room! <br> Please prepare your payment as you arrive.</div>
                  <?php } ?>
                  <?php if($row_reservation['status'] == 0) { ?>
                  <form action="web/route.php" method="POST">
                    ...
                    <button class="btn btn-danger float-end m-2 mb-4" name="cancel">Cancel</button>
                  </form>
                  <?php } ?>
                </div>
              </div>
